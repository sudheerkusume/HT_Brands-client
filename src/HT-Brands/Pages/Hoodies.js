...
const Hoodies = () => {
    ...
    useEffect(()=>{
        axios.get("http://localhost:4001/home2")
    ...
    <div className='row'>
 ...
 
        {
            Hoodies.map((Hoodies,index)=>{
                return(
                    <div key={index} className='col-md-4 mb-4'>
                        ...
                        {/* <button className='btn btn-primary ' style={{background : "bule", borderRadius: "100px", fontSize:"smaller"}}>Buy Now</button> */}
                            ...
                            <img className='Cover card-img-top h-100' src={Hoodies.image}  alt='error'/>
                ...
                <p className='card-text text-dark'><span className='linemarker pe-3'>{Hoodies.Price}</span> <span className='text-danger'>From {Hoodies.Discount}</span></p>
                <p className='text-warning'><strong>.</strong> {Hoodies.Stock}</p>
              </div>
                    </div>
                )
            })
